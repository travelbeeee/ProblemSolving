/*
	...
	우리는 2가지 정보가 필요
	현재 이모티콘의 개수, 클립보드에 복사해둔 이모티콘의 개수 

	1) 현재 화면의 이모티콘 개수 클립보드에 있는 개수 다르다면 클립보드에 새롭게 복사 가능
	2) 현재 클립보드에 있는 개수 0이 아니라면 현재 화면에 붙여넣기 가능
	3) 현재 화면에 이모티콘이 0개가 아니라면 하나 삭제 가능

	이렇게 3가지 동작을 수행할 수 있고, 구해야하는 이모티콘의 개수 S개라면
	클립보드에 S 를 넘어가는 값을 저장하는 경우는 항상 최소가 아니게 되고,
	화면에 있는 이모티콘의 개수 2 * S 가 넘어가는 경우도 최소가 아니게 된다.
